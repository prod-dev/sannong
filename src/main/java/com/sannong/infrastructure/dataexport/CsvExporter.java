...
public class CsvExporter {
    ...
    public static void doExport(HttpServletResponse response,List<Answer> answer) throws IOException {
        ...
        writer.write("编号,姓名,手机号码,工作单位," + header.toString() + "\n");
        ...
        while(it.hasNext()){
            ...
            writer.write( serialNumber + ","  +app.getApplicant().getRealName() + ","+ app.getApplicant().getCellphone()  + ","+ app.getApplicant().getCompanyAddress() + "," + CsvExporter.toAnalyseString(answers) + "\n");
            serialNumber++;
            writer.flush();
        }
        writer.close();
